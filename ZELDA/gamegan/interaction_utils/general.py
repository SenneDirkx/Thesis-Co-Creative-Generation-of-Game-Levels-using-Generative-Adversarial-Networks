...
def list_latent_to_tensor(list_level):
    return torch.tensor(list_level, dtype=torch.float32)

def list_level_to_tensor(list_level):
    for i in range(len(list_level)):
        for j in range(len(list_level[0])):
            zeros = [0, 0, 0, 0]
            zeros[list_level[i][j]] = 1
            list_level[i][j] = zeros
    
    tensor_level = torch.tensor(list_level, dtype=torch.float32)
